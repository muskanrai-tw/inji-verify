...
const getValue = (credentialElement: any, language: string = i18next.language || "en" ): string | undefined => {
  ...
  if (Array.isArray(credentialElement)) {
      const langEntry = credentialElement.find((el) => el?.["@language"] === language || el?.language === language);
        if (langEntry) {
  ...
  switch (type) {
      ...
      return getVCRenderOrders().MosipVerifiableCredentialRenderOrder.map((key: any) => {
        if (key in credential) {
          ...
          if(typeof(credential[key])=="object"){
            return { key, value: getValue(credential[key],language) } ;
          }
          return { key, value: getValue(credential[key], language) || "N/A" };
        }
        return { key, value: "N/A" };
      });
    ...
    case "SdJwtVC":
      return Object.keys(credential)
        ...
        .map((key) => ({ key, value: getValue(credential[key], language) }));
    default:
      return Object.keys(credential)
        .filter((key) =>
            key !== "id" &&
